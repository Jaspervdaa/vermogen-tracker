...
const DEFAULT_SNAPSHOTS = [
  {date:"2026-04-20",values:S1V,regularInleg:0,  extraStorting:0,extraStortingDate:""},
  {date:"2026-05-14",values:S2V,regularInleg:500,extraStorting:0,extraStortingDate:""},
];
...
const discreteExpectedFV = (start, fromDate, toDate, monthly, ar, depositDay=20) => {
  ...
  while(d<=to){
    const mRem=monthsBetween(d.toISOString().slice(0,10),toDate);
    result+=monthly*Math.pow(1+mr,mRem);
...
const expectedNWCorrect = (firstSnap, months, monthly, ar, bkts) => {
  ...
  const rest=bkts.reduce((s,b)=>{
    const sign=GROUPS.find(g=>g.id===b.group)?.sign??1;
    return b.group==="fin_vaste"?s:s+sign*Number(firstSnap.values[b.id]||0);
  },0);
...
const TT = ({active,payload,label}) => {
  ...
  const displayLabel=payload[0]?.payload?.fullDate?fmtDate(payload[0].payload.fullDate):fmtDate(label);
  return (
    <div style={{background:CARD2,border:"1px solid #2a3a55",borderRadius:8,padding:"10px 14px",fontSize:12}}>
      <div style={{color:"#8899bb",marginBottom:6,fontWeight:600}}>{displayLabel}</div>
...
const SpaarTT = ({active,payload,label}) => {
  if(!active||!payload?.length) return null;
  const d=payload[0]?.payload; if(!d) return null;
  const displayLabel=d.fullDate?fmtDate(d.fullDate):fmtDate(label);
...
// ── Firebase helpers ──
const saveToFirebase = async (uid, key, value) => {
  try {
    await setDoc(doc(db,"users",uid,"data",key),{value:JSON.stringify(value)},{merge:true});
  } catch(e){ console.error("Save error",e); }
};

const loadFromFirebase = async (uid, key) => {
  try {
...
export default function App() {
  ...
  const [newSnap,     setNewSnap]     = useState({date:new Date().toISOString().slice(0,10),values:{...S2V},regularInleg:500,extraStorting:0,extraStortingDate:""});
  ...
  const chartData = (()=>{
    ...
    for(let i=1;i<=12;i++){
      ...
      data.push({label:d.toISOString().slice(0,7),werkelijk:null,verwacht:expectedNWCorrect(first,tot+i,monthly,annualReturn/100,buckets)});
    }
    return data;
  })();
  ...
  const totaleKb=startKb+sorted.slice(1).reduce((s,x)=>s+Number(x.regularInleg||0)+Number(x.extraStorting||0),0);
  ...
  const periodeData=(()=>{
    ...
    return sorted.slice(1).map((s,i)=>{
      ...
      const regularInleg=Number(s.regularInleg||0), extraStorting=Number(s.extraStorting||0);
      const extraDate=s.extraStortingDate||s.date;
      const monthsExtra=Math.max(0,monthsBetween(extraDate,s.date));
      const totaleInleg=regularInleg+extraStorting;
      const verwachtEinde=discreteExpectedFV(prevFv,prev.date,s.date,monthly,annualReturn/100)+extraStorting*Math.pow(1+mr,monthsExtra);
      const verwachtZonderRendement=prevFv+totaleInleg;
      const marktResult=werkelijk-verwachtEinde;
      ...
      return {van:prev.date,tot:s.date,months,prevFv,werkelijk,verwacht:Math.round(verwachtEinde),
              verwachtZonderRendement,totaleInleg,regularInleg,extraStorting,extraDate,monthsExtra,
  ...
  const beleggingData=(()=>{
    ...
    sorted.forEach((s,i)=>{
      ...
      const extraStorting=Number(s.extraStorting||0), extraDate=s.extraStortingDate||s.date;
      for(let m=1;m<Math.floor(months);m++){
        ...
        if(new Date(dStr)>=new Date(extraDate)) v+=extraStorting*Math.pow(1+mr,Math.max(0,monthsBetween(extraDate,dStr)));
        data.push({label:d.toISOString().slice(0,7),werkelijk:null,verwacht:Math.round(v)});
      }
      const endVerwacht=discreteExpectedFV(prevFv,prev.date,s.date,monthly,annualReturn/100)+extraStorting*Math.pow(1+mr,Math.max(0,monthsBetween(extraDate,s.date)));
    ...
    for(let i=1;i<=12;i++){
      const d=new Date(first.date); d.setMonth(d.getMonth()+tot+i);
      data.push({label:d.toISOString().slice(0,7),werkelijk:null,verwacht:expectedFV(latestFv,i,monthly,annualReturn/100)});
    }
    return data;
  })();
  ...
  const spaarData=(()=>{
    if(!first) return [];
    let cumInleg=0;
    const snapshotDates=new Set(sorted.map(s=>s.date.slice(0,7)));
    ...
    for(let i=1;i<=12;i++){
      ...
      if(!snapshotDates.has(lbl)) data.push({label:lbl,werkelijk:null,doel:monthly*(tot+i),isSnapshot:false});
    }
    return data;
  })();
  ...
  const saveSnap=()=>{
    ...
    const snap={...newSnap,regularInleg:Number(newSnap.regularInleg||0),extraStorting:Number(newSnap.extraStorting||0)};
    ...
    setNewSnap({date:new Date().toISOString().slice(0,10),values:{},regularInleg:monthly,extraStorting:0,extraStortingDate:""});
    setView("overzicht");
  };

  const startEdit=i=>{
    const s=sorted[i], oi=snapshots.findIndex(x=>x.date===s.date);
    setEditIdx(oi); setNewSnap({regularInleg:monthly,extraStorting:0,extraStortingDate:"",...s,values:{...s.values}}); setView("snapshot");
  };
  ...
  if(authLoading) return (
    <div style={{minHeight:"100vh",background:BG,display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontFamily:"'Inter',system-ui,sans-serif"}}>
      <div style={{fontSize:14,color:"#8899aa"}}>Laden...</div>
    </div>
  );
  ...
  if(!user) return (
    <div style={{minHeight:"100vh",background:BG,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',system-ui,sans-serif"}}>
      <div style={{background:CARD,borderRadius:20,padding:48,textAlign:"center",border:"1px solid #1e3050",maxWidth:380}}>
        ...
        <div style={{fontSize:13,color:"#8899aa",marginBottom:32,lineHeight:1.6}}>Log in met je Google account om je vermogenspositie bij te houden. Je data is veilig opgeslagen en beschikbaar op al je apparaten.</div>
  ...
  // ── Hoofd app (zelfde layout als voorheen, nu met logout knop) ──
  return (
    <div style={{minHeight:"100vh",background:BG,color:"white",fontFamily:"'Inter',system-ui,sans-serif",fontSize:14}}>
      ...
      <div style={{background:"#080f1e",padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid #1a2a40",position:"sticky",top:0,zIndex:10}}>
        <div>
          ...
          {latest&&<div style={{fontSize:10,color:"#445566",marginTop:3}}>Laatste snapshot: {latest.date}</div>}
        </div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <NavBtn v="overzicht" label="Overzicht"/>
          <NavBtn v="snapshot" label="Snapshot" onClick={()=>{
            setView("snapshot");
            if(editIdx===null) setNewSnap({date:new Date().toISOString().slice(0,10),values:latest?{...latest.values}:{...S2V},regularInleg:monthly,extraStorting:0,extraStortingDate:""});
          }}/>
      ...
      <div style={{padding:20,maxWidth:1200,margin:"0 auto"}}>
        ...
        {view==="overzicht"&&(
          sorted.length===0
          ?(<div style={{...card(),textAlign:"center",padding:60,color:"#8899aa"}}>
              <div style={{fontSize:15,fontWeight:700,color:"white",marginBottom:8}}>Nog geen snapshots</div>
              <button onClick={()=>setView("snapshot")} style={{background:TEAL,color:"#080f1e",border:"none",borderRadius:8,padding:"10px 28px",fontWeight:700,cursor:"pointer",fontSize:13,marginTop:12}}>Snapshot toevoegen</button>
            </div>)
          :(<div style={{display:"flex",flexDirection:"column",gap:14}}>

              {/* KPI */}
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>
                <div style={{...card(),background:"linear-gradient(135deg,#7c5cfc,#00e5cc)",border:"none"}}>
                  <div style={{fontSize:11,opacity:.8,marginBottom:4}}>Eigen Vermogen</div>
                  <div style={{fontSize:26,fontWeight:800}}>{fmt(latestNW)}</div>
                  <div style={{fontSize:11,opacity:.6,marginTop:4}}>{latest?.date}</div>
                </div>
                <div style={card()}>
                  <div style={{fontSize:11,color:"#8899aa",marginBottom:4}}>Vs. Verwachting</div>
                  <div style={{fontSize:24,fontWeight:800,color:diff>=0?TEAL:RED}}>{diff>=0?"+":""}{fmt(diff)}</div>
                  <div style={{fontSize:11,color:"#8899aa",marginTop:4}}>{diff>=0?"Voor op schema":"Achter op schema"}</div>
                </div>
                <div style={card()}>
                  <div style={{fontSize:11,color:"#8899aa",marginBottom:6}}>Inleg per maand</div>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <span style={{color:"#8899aa",fontSize:16}}>€</span>
                    <input type="number" value={monthly} onChange={e=>setMonthly(Number(e.target.value))} style={{background:"transparent",border:"none",borderBottom:`1px solid ${PURP}`,color:PURP,fontSize:22,fontWeight:800,width:"100%",outline:"none",padding:"2px 0"}}/>
                  </div>
                  <div style={{fontSize:11,color:"#8899aa",marginTop:6}}>{annualReturn}% rendement / jaar</div>
                </div>
                <div style={card()}>
                  <div style={{fontSize:11,color:"#8899aa",marginBottom:4}}>Verwacht over 12 mnd</div>
                  <div style={{fontSize:24,fontWeight:800,color:ORG}}>{fmt(expectedNWCorrect(latest?{date:latest.date,values:latest.values}:null,12,monthly,annualReturn/100,buckets))}</div>
                  <div style={{fontSize:11,color:"#8899aa",marginTop:4}}>prognose</div>
                </div>
              </div>

              {/* Totaal vermogen grafiek */}
              <div style={card()}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  <div style={{fontSize:14,fontWeight:700}}>Totaal Eigen Vermogen</div>
                  <div style={{display:"flex",gap:20,fontSize:11,color:"#8899aa"}}>
                    <span><span style={{color:TEAL}}>—</span> Werkelijk</span>
                    <span><span style={{color:PURP}}>- -</span> Verwacht</span>
                  </div>
                </div>
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={chartData}>
                    ...
                    <Line type="monotone" dataKey="werkelijk" name="Werkelijk" stroke={TEAL} strokeWidth={3} dot={{fill:TEAL,r:5}} connectNulls={false}/>
                    <Line type="monotone" dataKey="verwacht"  name="Verwacht"  stroke={PURP} strokeWidth={2} strokeDasharray="6 4" dot={false}/>
                  </LineChart>
                </ResponsiveContainer>
              </div>
              ...
              <div style={card()}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                  <div>
                    <div style={{fontSize:14,fontWeight:700}}>Beleggingen — Werkelijk vs. Verwacht</div>
                    <div style={{fontSize:11,color:"#556677",marginTop:2}}>Extra storting rendeert vanaf exact de opgegeven datum · inleg op de 20ste · {annualReturn}%</div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:10,color:"#8899aa",letterSpacing:.4}}>TOTAAL MARKTRESULTAAT</div>
                    <div style={{fontSize:18,fontWeight:800,color:totaalMarktResult>=0?TEAL:RED}}>{totaalMarktResult>=0?"+":""}{fmt(totaalMarktResult)}</div>
                    <div style={{fontSize:10,color:"#556677"}}>op kostenbasis {fmt(totaleKb)}</div>
                  </div>
                </div>
                <ResponsiveContainer width="100%" height={180}>
                  <LineChart data={beleggingData}>
                    ...
                    <Tooltip content={<TT/>}/>
                    <Line type="monotone" dataKey="werkelijk" name="Werkelijk" stroke={PURP} strokeWidth={3} dot={{fill:PURP,r:5}} connectNulls={false}/>
                    <Line type="monotone" dataKey="verwacht"  name="Verwacht"  stroke={ORG}  strokeWidth={2} strokeDasharray="6 4" dot={false}/>
                  </LineChart>
                </ResponsiveContainer>
                  ...
                  <div style={{marginTop:20}}>
                    ...
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
                      <thead><tr style={{borderBottom:"1px solid #1e3050"}}>
                        {["Periode","Waarde begin","Reg. inleg","Extra (+ datum)","Verwacht einde","Werkelijk","Marktresultaat","Rendement"].map(h=>(
                          <th key={h} style={{textAlign:h==="Periode"?"left":"right",padding:"7px 10px",fontSize:10,color:"#667799",fontWeight:600}}>{h}</th>
                        ))}
                      </tr></thead>
                      <tbody>
                        {periodeData.map((p,i)=>{
                          const pos=p.marktResult>=0;
                          return (<tr key={i} style={{borderBottom:"1px solid #131f35"}}>
                            ...
                            <td style={{padding:"9px 10px",textAlign:"right",fontSize:11,color:p.extraStorting>0?BLUE:"#445566"}}>{p.extraStorting>0?<span>{fmt(p.extraStorting)}<br/><span style={{fontSize:9,color:"#667799"}}>{fmtDate(p.extraDate)}</span></span>:"—"}</td>
              ...
              <div style={card()}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                  <div>
                    <div style={{fontSize:14,fontWeight:700}}>Spaardiscipline — Reguliere Inleg</div>
                    <div style={{fontSize:11,color:"#556677",marginTop:2}}>Cumulatief ingelegd vs. doel van €{monthly}/mnd</div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:10,color:"#8899aa",letterSpacing:.4}}>HUIDIGE POSITIE</div>
                    <div style={{fontSize:18,fontWeight:800,color:inlegVerschilTotaal>=0?TEAL:RED}}>{inlegVerschilTotaal>=0?"+":""}{fmt(Math.round(inlegVerschilTotaal))}</div>
                    <div style={{fontSize:10,color:"#556677"}}>t.o.v. doel {fmt(Math.round(inlegDoel))}</div>
                  </div>
                </div>
                <ResponsiveContainer width="100%" height={180}>
                  <LineChart data={spaarData}>
                    ...
                    <ReferenceLine y={0} stroke="#2a3a55"/>
                    <Line type="monotone" dataKey="werkelijk" name="Werkelijk" stroke={GREEN} strokeWidth={3} dot={d=>d.payload?.isSnapshot?<circle key={d.key} cx={d.cx} cy={d.cy} r={5} fill={GREEN}/>:null} connectNulls={false}/>
                    <Line type="monotone" dataKey="doel"      name="Doel"      stroke={ORG}   strokeWidth={2} strokeDasharray="6 4" dot={false}/>
                  </LineChart>
                </ResponsiveContainer>
              </div>
              ...
              {/* Balans + Donut */}
              {(()=>{
                const totalVaste=vasteGroups.reduce((s,g)=>s+calcGroup(latest.values,g),0);
                const totalVlot=vlotGroups.reduce((s,g)=>s+calcGroup(latest.values,g),0);
                ...
                return (
                  <div style={{display:"grid",gridTemplateColumns:"1.2fr 1fr",gap:14}}>
                    <div style={card()}>
                      ...
                      <div style={{fontSize:10,color:"#556677",marginBottom:14}}>Peildatum: {latest.date}</div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
                        <div>
                          ...
                          {[vasteGroups,vlotGroups].map((grp,gi)=>(
                            <div key={gi}>
                              <div style={{fontSize:10,fontWeight:700,color:"#667799",marginTop:8,marginBottom:4}}>{gi===0?"Vaste activa":"Vlottende activa"}</div>
                              {grp.map(gid=>{
                                const g=GROUPS.find(x=>x.id===gid), items=buckets.filter(b=>b.group===gid), groupTotal=calcGroup(latest.values,gid);
                          ...
                          <div style={{marginTop:10,padding:"8px 0",borderTop:"2px solid #2a3a55",display:"flex",justifyContent:"space-between"}}>
                            <div style={{fontSize:12,fontWeight:800,color:"white"}}>Totaal activa</div>
                            <div style={{fontSize:12,fontWeight:800,color:TEAL}}>{fmt(totalActiva)}</div>
                          </div>
                        </div>
                        <div>
                          ...
                          <div style={{marginTop:10,padding:"8px 0",borderTop:"2px solid #2a3a55",display:"flex",justifyContent:"space-between"}}>
                            <div style={{fontSize:12,fontWeight:800,color:"white"}}>Totaal passiva</div>
                            <div style={{fontSize:12,fontWeight:800,color:PURP}}>{fmt(totalSchuld+eigenVerm)}</div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div style={card()}>
                      ...
                      <ResponsiveContainer width="100%" height={180}>
                        <PieChart><Pie data={pieData} dataKey="value" innerRadius={55} outerRadius={82} paddingAngle={3} startAngle={90} endAngle={-270}>{pieData.map((d,i)=><Cell key={i} fill={d.color}/>)}</Pie>
                          <Tooltip formatter={v=>fmt(v)} contentStyle={{background:CARD2,border:"1px solid #2a3a55",borderRadius:8,fontSize:12}}/>
                        </PieChart>
                      </ResponsiveContainer>
                      <div style={{marginTop:8}}>
                        {pieData.map((d,i)=>{const pct=Math.round((d.value/totalActiva)*100);return(<div key={i} style={{marginBottom:10}}>
                          <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:11,color:"#aabbcc"}}>{d.name}</span><span style={{fontSize:11,fontWeight:700,color:d.color}}>{fmt(d.value)} <span style={{color:"#667799"}}>({pct}%)</span></span></div>
                          <div style={{background:"#1a2a40",borderRadius:3,height:4}}><div style={{background:d.color,width:`${pct}%`,height:"100%",borderRadius:3}}/></div>
                        </div>);})}
                      </div>
                    </div>
                  </div>
              ...
              <div style={card()}>
                ...
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  <thead><tr style={{borderBottom:"1px solid #1e3050"}}>
                    {["Datum","Eigen Vermogen","Groei periode","Vs. Verwacht",""].map(h=>(
                      <th key={h} style={{textAlign:h==="Datum"?"left":"right",padding:"8px 10px",fontSize:10,color:"#667799",fontWeight:600,letterSpacing:.4}}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {sorted.map((s,i)=>{
                      const nw=calcNW(s.values), prevNW=i>0?calcNW(sorted[i-1].values):null, groei=prevNW!==null?nw-prevNW:null;
                      const months=first?monthsBetween(first.date,s.date):0;
                      const exp=i===0?nw:expectedNWCorrect(first,months,monthly,annualReturn/100,buckets), d=nw-exp;
            ...
            <div style={card()}>
              ...
              {GROUPS.filter(g=>g.side==="activa").map(g=>{
                ...
                return (<div key={g.id} style={{marginBottom:20}}>
                  ...
                  {items.map(b=>(<div key={b.id} style={{display:"grid",gridTemplateColumns:"1fr 150px 28px",gap:8,alignItems:"center",marginBottom:10}}>
                    ...
                    <div style={{position:"relative"}}><span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#8899aa",fontSize:13}}>€</span><input type="number" placeholder="0" value={newSnap.values[b.id]??""} onChange={e=>setNewSnap(p=>({...p,values:{...p.values,[b.id]:e.target.value}}))} style={inp({paddingLeft:24,textAlign:"right"})}/></div>
              ...
              <div style={{background:CARD2,borderRadius:10,padding:14,marginBottom:20}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                  <div>
                    <label style={{fontSize:11,color:GREEN,display:"block",marginBottom:5,fontWeight:600}}>REGULIERE INLEG</label>
                    <div style={{position:"relative"}}><span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#8899aa",fontSize:13}}>€</span><input type="number" min="0" placeholder="0" value={newSnap.regularInleg??""} onChange={e=>setNewSnap(p=>({...p,regularInleg:e.target.value}))} style={inp({paddingLeft:24,textAlign:"right"})}/></div>
                    <div style={{fontSize:10,color:"#445566",marginTop:4}}>Vanuit inkomen — telt mee voor spaardiscipline</div>
                  </div>
                  <div>
                    <label style={{fontSize:11,color:BLUE,display:"block",marginBottom:5,fontWeight:600}}>EXTRA STORTING</label>
                    <div style={{position:"relative"}}><span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#8899aa",fontSize:13}}>€</span><input type="number" min="0" placeholder="0" value={newSnap.extraStorting??""} onChange={e=>setNewSnap(p=>({...p,extraStorting:e.target.value}))} style={inp({paddingLeft:24,textAlign:"right"})}/></div>
                    <div style={{fontSize:10,color:"#445566",marginTop:4}}>Bijv. terugbetaling — neutrale verschuiving</div>
                  </div>
                </div>
                {Number(newSnap.extraStorting||0)>0&&(<div>
                  <label style={{fontSize:11,color:BLUE,display:"block",marginBottom:5,fontWeight:600}}>DATUM EXTRA STORTING</label>
                  <input type="date" value={newSnap.extraStortingDate||newSnap.date} onChange={e=>setNewSnap(p=>({...p,extraStortingDate:e.target.value}))} style={inp()}/>
                  <div style={{fontSize:10,color:"#445566",marginTop:4}}>Wanneer is dit geld ingelegd? Rendeert vanaf deze datum.</div>
                </div>)}
              </div>
              ...
              {GROUPS.filter(g=>g.side==="passiva").map(g=>{
                ...
                return (<div key={g.id} style={{marginBottom:20}}>
                  {items.map(b=>(<div key={b.id} style={{display:"grid",gridTemplateColumns:"1fr 150px 28px",gap:8,alignItems:"center",marginBottom:10}}>
                    <div><div style={{fontSize:12,fontWeight:500}}>{b.name}</div>{b.note&&<div style={{fontSize:10,color:"#667799"}}>{b.note}</div>}</div>
                    <div style={{position:"relative"}}><span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#8899aa",fontSize:13}}>€</span><input type="number" min="0" placeholder="0" value={newSnap.values[b.id]??""} onChange={e=>setNewSnap(p=>({...p,values:{...p.values,[b.id]:e.target.value}}))} style={inp({paddingLeft:24,textAlign:"right"})}/></div>
                    <DeleteBtn onClick={()=>deleteBucket(b.id)}/>
                  </div>))}
          ...
          <div style={{maxWidth:620,margin:"0 auto",display:"flex",flexDirection:"column",gap:14}}>
            <div style={card()}>
              ...
              <div style={{marginBottom:14}}>
                <label style={{fontSize:11,color:"#8899aa",display:"block",marginBottom:5,letterSpacing:.4}}>MAANDELIJKS INLEGGEN (€)</label>
                <input type="number" value={monthly} onChange={e=>setMonthly(Number(e.target.value))} style={inp()}/>
              </div>
              <div>
                <label style={{fontSize:11,color:"#8899aa",display:"block",marginBottom:5,letterSpacing:.4}}>VERWACHT JAARRENDEMENT (%)</label>
                <input type="number" value={annualReturn} step="0.5" onChange={e=>setAnnualReturn(Number(e.target.value))} style={inp()}/>
                <div style={{fontSize:11,color:"#445566",marginTop:6}}>Vanguard All-World historisch gemiddelde ≈ 7–9% per jaar</div>
              </div>
            </div>
            <div style={card()}>
              ...
              <input type="number" value={startKb} onChange={e=>setStartKb(Number(e.target.value))} style={inp()}/>
              <div style={{fontSize:11,color:"#445566",marginTop:6}}>Begin: {fmt(startKb)} · Totaal nu: {fmt(totaleKb)}</div>
            </div>
            <div style={card()}>
              <div style={{fontSize:14,fontWeight:700,marginBottom:16}}>Posten beheren</div>
              {GROUPS.map(g=>(<div key={g.id} style={{marginBottom:20}}>
                <div style={{fontSize:11,fontWeight:700,color:g.color,marginBottom:8,letterSpacing:.4}}>{g.label.toUpperCase()}</div>
                {buckets.filter(b=>b.group===g.id).map(b=>(<div key={b.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:"1px solid #1a2a40"}}>
                  <div style={{flex:1}}><div style={{fontSize:12,fontWeight:500}}>{b.name}</div>{b.note&&<div style={{fontSize:10,color:"#667799"}}>{b.note}</div>}</div>
                  <DeleteBtn onClick={()=>deleteBucket(b.id)}/>
                </div>))}
